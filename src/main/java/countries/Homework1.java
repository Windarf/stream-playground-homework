...
public class Homework1 {
    ...
    public void streamPipeline3() {
        countries.stream()
                .filter(country -> country.getName().toLowerCase().charAt(0) ==
                        country.getName().toLowerCase().charAt(country.getName().length()-1))
                .map(country -> country.getName())
    ...
    public void streamPipeline5() {
        countries.stream()
                .sorted(Comparator.comparingLong(country -> country.getPopulation()))
                .limit(10)
                .map(country -> country.getName())
                .forEach(System.out::println);
    }
    ...
    public IntSummaryStatistics streamPipeline6() {
        return countries.stream()
                .mapToInt(coutry -> coutry.getTranslations().size())
                .summaryStatistics();
    }
...
ed by the language code "es").
    ...
    public void streamPipeline15() {
        countries.stream()
                .filter(country -> country.getArea() != null &&
                        country.getArea().compareTo(new BigDecimal(1)) < 1)
                .map(country -> country.getName())
                .forEach(System.out::println);
    }
